...
9. Near the center of the page, click on *Global credentials (unrestricted)*

![global creds](./images/globalCreds.png)

10. Then, click on *Add credential* on the left side of the page.

11. Enter the following information for the credential:
...
7. Change the **Script Path** to `Jenkinsfile.iks`

![pipeline configuration](./images/pipelineConfig.png)

4. Scroll up to the **Build Trigger** section and select **GitHub hook trigger for GIT SCM polling**

![Build Triggers](images/ss2.png)

6. Before we kick off our pipeline, there are a few environment variables that we need to set. Scroll up and select the option that says *This project is parameterized*. Then click on the *Add Parameter* drop down that appears and select **string parameter**

![parameterized](./images/parameterized.png)

7. In the new parameter that comes up, enter the following information:
